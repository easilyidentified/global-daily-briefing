...
호스트 엘리먼트에 `overflow:hidden`이 없어서(`issue-graph.js:49-53`) 넘친 카드는
잘리는 대신 **그래프 테두리 밖 아래 콘텐츠 위로 겹쳐 나온다.**
...
- `cardLayer`에 `overflow:hidden` — 상한 계산이 틀려도 페이지 아래로는 절대 새지 않게 하는 안전망
- **부수 수정 2건**
