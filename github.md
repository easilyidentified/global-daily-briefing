...
**파이프라인 장애 복구 + 재시도 경로 추가 (세션 25)**

- **08-20 브리핑이 통째로 빠졌다.** 06:40 수집이 구독 5시간 한도(429)에 걸려 22초 만에 사망 —
  `"You've hit your session limit · resets 8:40am"`. 1턴도 못 돌았다
- `issues.json`이 08.19에 머물러 08:00 발송은 신선도 가드로 스스로 멈췄다.
  **발송 쪽은 설계대로 동작** — 어제치 재발송 사고는 없었다
- 진짜 약점은 **시도가 하루 한 번뿐**이라 복구 경로가 없었던 것. 한도 리셋은 08:40이었다
- **수집 09:40 / 발송 10:30 재시도 크론 추가** (`c345207`).
  수집은 4개국 `latestDate`가 모두 오늘이면 스텝을 건너뛰어 정상적인 날엔 쿼터를 안 쓴다.
  발송은 `sent.json` 멱등 가드가 중복을 막는다
- **수집 push 앞에 `git pull --rebase --autostash` 추가** (`e9d15a5`) —
  발송에는 있던 것이 수집에는 없었다. 수동 재수집 도중 main이 움직여
  push가 non-fast-forward로 거부되며 **수집분 한 벌을 실제로 잃고** 알았다
- 08-20치 복구 완료: 4개국 각 6건, KR 브리핑 **29명 발송 확인** (11:08 KST)
- **워크플로 success ≠ 발송됨.** 로그의 `[send] 발송 완료 → 배달 N명`을 봐야 한다.
  실패 원인도 Actions 로그가 아니라 `claude-execution-output` 아티팩트에 있다

**고객 조사 — `CUSTOMER.md` 신설 (세션 24)**

- 6단계 순서로 조사: 잠재 고객 → 예산 쓰며 던지는 질의 → 세그먼테이션 → 대응 방식 → pain → 해소 방식
- **세그먼트 7개.** 산업이 아니라 **질의 형태 × 틀렸을 때의 결과**로 잘랐다.
  같은 은행 안에 컴플라이언스(Q1)와 리서치(Q5)가 있고 둘은 다른 제품을 산다
- **결론: 세그먼트는 S3(정책·통상 보고서)로 시작, 엔티티는 S1(규제 대응 실사) 기준으로 설계**
- **S3만이 "대체재가 사람"이다.** 나머지는 전부 이미 벤더가 있다
- S5(금융)는 배제 — 균일 시계열이 필요해 `DB-DESIGN.md` 결정 E와 정면충돌
- S4(학계)는 수요는 확실하나 빅카인즈·GDELT가 무료. **검증 파트너로만**
- **빅카인즈가 국내 파트를 무료로 덮는다(국내 매체 한정).** 그래서 국가 간 대조가 차별점으로 성립
- **adverse media 오탐율 95% = "아키텍처 문제".** 주범은 동명이인 + 비라틴 음역.
  L2(사건 병합)가 푸는 문제와 같은 문장이다
- **되돌릴 수 없는 결정은 엔티티·속성 설계 하나뿐** — 추출 원문 보관은
  애초에 안 뽑은 속성을 커버하지 못한다
- 지금 없는데 필수인 3가지: 엔티티 `disambiguator`, `names[lang]`(한/영/중/일), 사건 `status`
- 당장 바꿀 한 줄: `topics: [문자열]` → `entities: [{id, role}]`. **`role`이 이번 추가**
- **경고: GDELT 핵심 필드 정확도 ~55%, 영어 편중, 중국 인트라넷 매체 누락.**
  한·중·일 커버리지를 GDELT에 의존하면 차별점이 서는 지점이 빈다

**`사용자제공/`을 `.gitignore`에 추가 (세션 24)**

- claude.ai Design 프로젝트 사본. 커밋하면 디자인 원본이 둘이 된다

**사이트 이슈 구간을 이메일 형태로 통합 (세션 23)**

- `Globe Briefing.dc.html` 한 파일. '오늘의 주요 이슈' 표 + '분야별 주요 소식 및 맥락 분석'
  → 이메일과 같은 `요약표 + 카드` 한 벌. 브랜치 `issue-layout-email`에서 작업 후 `--no-ff` 머지
- **`why`/`soWhat`이 카드 안으로 들어왔다.** 전에는 표에만 있어 카드만 읽으면 이유가 빠졌다.
  `ISSUE`/`IMPACT`는 **표시 라벨일 뿐 필드명이 아니다** — 데이터는 그대로
- 요약표가 정치/경제/기타 탭을 따라간다(오늘치만). 카드는 과거치까지, 날짜 레일 유지
- 함께 읽기는 **기존 박스 형태 유지**(사용자 결정). 출처만 이메일식 한 줄로
- 되돌리기: 태그 **`layout-v1-table`** (직전 화면 모양)
- 검증: 점프 3갈래 모두 `scrollTo` 인자 실측 일치. 세션 20 회귀 지점이라 직접 쟀다
- **모바일 미확인** — 창 리사이즈가 뷰포트에 반영되지 않음. 폰에서 봐야 한다

**DB 설계 문서 신설 → v2 개정 (세션 23)**

- `DB-DESIGN.md`. 사용자가 대규모 소급 수집을 전제로 정정 → 항목 구성 전면 재작성
- **저장은 병목이 아니다**: 438만건 = 8.4GB(월 300원) vs AI 추출 1,533만원. **5만 배**
- 1순위는 저장소가 아니라 **획득과 추출 예산**. 8가지 → **4개 층 11가지**
- 지금 수집 방식으로 438만건은 500년 → 넓고 얕은 무료 층 + 좁고 깊은 AI 추출 2단
- 결정: B 유지 확정 / D 자리만 / E 필요한 만큼 / A는 1순위에서 내림
- 아티팩트: `https://claude.ai/code/artifact/c16c8c1f-2ce6-4d11-82a5-f1669a0fab84`

**id 형식 불일치 발견 (세션 23)**

- `kr-a-pol-1` 형태가 국가당 6건씩 총 24건. 자동 수집 이전 초기 데이터(`review` 필드 없음)
- 지금 깨지는 곳 없음 — 리포 어디서도 id를 쪼개 쓰지 않는 것까지 확인
- DB 이관 때 id를 파싱하지 않으면 소급 수정 0건
